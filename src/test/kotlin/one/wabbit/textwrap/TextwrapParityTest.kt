...
class TextWrapParityTest {
    ...
    @Test fun wrap_initial_and_subsequent_indent() {
        val tw = TW(width = 12, initial = ">>> ", subsequent = "... ")
    ...
    @Test fun wrap_expand_tabs_and_replace_ws() {
        val tw = TW(width = 8, tabsize = 4)
        val text = "foo\tbar\n\nbaz    qux"
        val expected = listOf("foo bar", "baz", "qux")
        assertEquals(expected, tw.wrap(text))
    }

    @Test fun wrap_fix_sentence_endings() {
        val tw = TW(width = 20, fixSent = true)
        val text = "Hello world.\nBar baz? \"Quux\"!"
        val expected = listOf("Hello world.  Bar", "baz?  \"Quux\"!")
        assertEquals(expected, tw.wrap(text))
    }

    @Test fun wrap_break_long_words_true() {
        val tw = TW(width = 10, breakLong = true)
        val text = "Supercalifragilisticexpialidocious"
        val expected = listOf("Supercalif", "ragilistic", "expialidoc", "ious")
        assertEquals(expected, tw.wrap(text))
    }

    @Test fun wrap_break_long_words_false() {
        val tw = TW(width = 10, breakLong = false)
        val text = "Supercalifragilisticexpialidocious"
        val expected = listOf("Supercalifragilisticexpialidocious")
        assertEquals(expected, tw.wrap(text))
    }

    @Test fun wrap_break_on_hyphens_true() {
        val tw = TW(width = 10, breakOnHyphens = true)
        val text = "Look, goof-ball -- use the -b option!"
        val expected = listOf("Look,", "goof-ball", "-- use the", "-b option!")
        assertEquals(expected, tw.wrap(text))
    }

    @Test fun wrap_break_on_hyphens_false() {
        val tw = TW(width = 10, breakOnHyphens = false)
        val text = "Look, goof-ball -- use the -b option!"
        val expected = listOf("Look,", "goof-ball", "-- use the", "-b option!")
        assertEquals(expected, tw.wrap(text))
    }

    @Test fun wrap_drop_whitespace_behavior() {
        val tw = TW(width = 3, dropWS = true)
        val text = "  A  B   C  "
        val expected = listOf("  A", "B", "C")
        assertEquals(expected, tw.wrap(text))
    }

    @Test fun wrap_max_lines_with_placeholder() {
        val tw = TW(width = 12, maxLines = 2, placeholder = " [...]")
        val text = "alpha beta gamma delta epsilon zeta eta theta iota"
        val expected = listOf("alpha beta", "gamma [...]")
        assertEquals(expected, tw.wrap(text))
    }

    @Test fun wrap_unicode_letters_basic() {
        val tw = TW(width = 12)
    ...
    @Test fun wrap_nbsp_is_nonbreaking() {
        val tw = TW(width = 2)
        val text = "A\u00A0B C"
        val expected = listOf("A\u00A0", "B", "C")
        assertEquals(expected, tw.wrap(text))
    ...
    @Test fun fill_fix_sentence_endings() {
        val tw = TW(width = 20, fixSent = true)
        val text = "Hello world.\nBar baz? \"Quux\"!"
    ...
    @Test fun indent_default_predicate() {
        val text = "a\n\nb\n"
        val expected = "> a\n\n> b\n"
        assertEquals(expected, indent(text, "> "))
    }

    @Test fun indent_predicate_startswith_b() {
        val text = "a\nb\nc\n"
        val expected = "a\n# b\nc\n"
